...
static DivStatus extract_oprtn(char* min,
                               char* oprtr, char* prev_oprtr,
                               EqAr* eq, Bounds* b)
{
    ...
    parse_oprtn(eq->op2_num_s, NULL, eq->op2_den_s,
                op1, '\0', op2);
    eq->oprtr = *prev_oprtr;
...
static DivStatus extract_oprtn_l(char* s, char* start,
                                 char* prev_oprtr,
                                 EqAr* eq, Bounds* b)
{
...
static DivStatus extract_oprtn_r(char* start,
                                 char* prev_oprtr,
                                 EqAr* eq, Bounds* b)
{
                               ...
                               EqAr* eq, Bounds* b)
{
    switch (extract(s, eq, b)) {
        case FAIL:
            ...
            return true;
        case PARSED_TO_PREV:
            parse_oprtn(eq->op1_num_s, NULL, eq->op1_den_s,
                        op1, '\0', op2);
            return true;
        case PARSED_TO_NEW:
            return true;
